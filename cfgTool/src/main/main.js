...
const { generateScripts } = require('./script-generators');
...
ipcMain.handle('export-project', async (_event, payload = {}) => {
  const { configDir, annotationDir, jsonDir, scriptDir, scriptLanguage } = payload;
  ...
  try {
    ...
    // 3. 压缩并写入二进制文件
    if (binGroups.size === 0) {
      await writeCompressedBin(jsonDir, 'gamedata.bin', {});
    } else {
      for (const [binFile, groupData] of binGroups) {
        await writeCompressedBin(jsonDir, binFile, groupData);
      }
    }
